...
class LMap {
  ...
  new_map($el) {
    ...
    L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
      ...
      tileSize: 512,
      zoomOffset: -1,
      accessToken: php_vars.mapbox_access_token
    }).addTo(map);
    map.markers = [];
...
class Like {
  constructor() {
    ...
    if (this.likeBox) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults.headers.common["X-WP-Nonce"]) = bcodingData.nonce;
      this.events();
    }
  }
...
class MyNotes {
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(`${bcodingData.root_url}/wp-json/wp/v2/note/`, newNote);
      console.log(response.data);
            ...
            <div class="row">
                ...
                <div class="one-third">
                    ...
                    <h2 class="search-overlay__section-title">Events</h2>
                        ${!results.events.length ? `<p>No events matches your search. <a href="${bcodingData.root_url}/events">View all events.</a></p>` : `${results.events.map(result => `
...
class TableSearch {
  ...
  searchPets() {
    ...
    query('tbody tr').forEach(row => row.style.display = filters.every((f, i) => f.test(row.cells[i].textContent)) ? '' : 'none');
  }
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkletsbcoding_theme"] = self["webpackChunkletsbcoding_theme"] || [];
